...
import json, os, multiprocessing, psutil, time, sys
...
def main():
 ...
 p=ctx.Process(target=app.run_background_job_with_fatal_guard,args=(app.run_resilient_collection_job,cfg,sig,q,jid,str(d/'logs'/'run.log'))); p.start(); samples={}; started=time.time()
 while p.is_alive():
  elapsed=time.time()-started
  if elapsed>120: p.terminate(); break
  if int(elapsed)%5==0: samples.setdefault(str(int(elapsed)),psutil.Process(p.pid).memory_info().rss//1048576)
  time.sleep(1)
 p.join(); report={'exit_code':p.exitcode,'samples_mb':samples,'data_dir':str(d)}; (d/'durability_report.json').write_text(json.dumps(report,indent=2)); print(json.dumps(report,indent=2)); return 0 if p.exitcode==0 else 1
