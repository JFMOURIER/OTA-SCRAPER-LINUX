...
class SearchApiGoogleHotelsCollector(BaseCollector):
    source_name = "SearchAPI Google Hotels"
    ...
    def collect(
        ...
        if not api_key:
            message = "SEARCHAPI_API_KEY is missing. Add it to .env to use this source."
        ...
        self.progress(progress_callback, 0.1, "Calling SearchAPI Google Hotels")
        self.log(log_callback, "Calling SearchAPI Google Hotels API.")
        try:
            payload = self._get_json(url)
            options.stats["api_requests_used"] = int(options.stats.get("api_requests_used", 0)) + 1
        except Exception as exc:
            message = f"SearchAPI Google Hotels request failed: {exc}"
            self.log(log_callback, message)
            return self.unavailable_result(message, city_or_region, checkin_date, checkout_date, adults, currency)
        ...
        for index, hotel in enumerate(hotels, start=1):
            rows.append(
                self.normalized_row(
                    ...
                    cheapest_price_total=self._price_total(hotel),
            ...
            self.progress(progress_callback, 0.15 + (index / max(len(hotels), 1)) * 0.8, f"Parsed SearchAPI hotel {index} of {len(hotels)}")

        if not rows:
            rows = self.unavailable_result("SearchAPI returned no hotel rows for this search.", city_or_region, checkin_date, checkout_date, adults, currency)
        self.progress(progress_callback, 1.0, f"SearchAPI complete: {len(rows)} row(s)")
        return rows
